...
export default function Home() {
  ...
  const renderNotes = (notes, targetElement) => {
    ...
    notes.forEach((note) => {
      ...
      const actionButtons = note.archived
        ? `
          <button id="${note.id}" class="btn blue btn-unarchive" aria-label="Batal Arsip Jurnal
        ...
        `
        : `
          <button id="${note.id}" class="btn btn-archive" aria-label="Arsip jurnal">
      ...
      article.innerHTML = `
        ...
        <div class="action">
          ${actionButtons}
          <button id="${note.id}" class="btn red btn-delete" aria-label="Hapus Jurnals">
    ...
    document.querySelectorAll('.btn-archive').forEach((btn) => {
      btn.addEventListener('click', async (event) => {
        try {
          ...
          hideLoading(loading);
          Swal.fire('Gagal Mengarsipkan Jurnal', `${error.message}`, 'error');
        } finally {
          hideLoading(loading);
        }
      });
    ...
    document.querySelectorAll('.btn-unarchive').forEach((btn) => {
      btn.addEventListener('click', async (event) => {
        try {
          ...
          const { message } = await response.json();
          updateNotesDisplay();
          Swal.fire('Jurnal telah dikembalikan!', `${message}`, 'success');
        } catch (error) {
          hideLoading(loading);
          Swal.fire('Gagal mengembalikan Jurnal', `${error.message}`, 'error');
        } finally {
          hideLoading(loading);
        }
      });
    ...
    btnDelete.forEach((btn) => {
      btn.addEventListener('click', async (event) => {
        Swal.fire({
          ...
          text: 'Jurnal yang telah dihapus tidak dapat dikembalikan!',
          icon: 'question',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          confirmButtonText: 'Apakah Anda Yakin?',
        }).then(async (result) => {
          if (result.isConfirmed) {
            try {
              ...
              hideLoading(loading);
              Swal.fire('Gagal menghapus Jurnal', `${error.message}`, 'error');
            } finally {
              hideLoading(loading);
            }
          }
  ...
  const updateNotesDisplay = async (searchQuery = '') => {
    try {
      ...
      Swal.fire('Gagal memuat Jurnal', `${error.message}`, 'error');
    } finally {
      hideLoading(loading);
    }
  };
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (status === 'success') {
      Swal.fire({
        title: 'jurnal Telah Berhasil Ditambahkan',
        text: `${message}`,
        icon: 'success',
        confirmButtonColor: '#3085d6',
      });
      ...
      Swal.fire({
        title: 'Gagal Saat Menambahkan Jurnal',
        text: `${message}`,
        icon: 'error',
        confirmButtonColor: '#3085d6',
      });
